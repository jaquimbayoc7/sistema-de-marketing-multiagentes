...
def agent_market_analyst(state: MarketingProposalState) -> dict:
    """Analiza al cliente, su mercado, pain points y oportunidades usando el framework PEST + Pain-Gain-Fear."""
...
def agent_brand_strategist(state: MarketingProposalState) -> dict:
    """Define mensajes clave, propuesta de valor y estrategia persuasiva usando el Messaging Hierarchy Canvas."""
    llm = _llm(state["api_key"])
    messages = [
...
def agent_editor(state: MarketingProposalState) -> dict:
    """Revisa y pule la propuesta aplicando la rubrica PACT para máximo impacto."""
    llm = _llm(state["api_key"], max_tokens=4096)
    messages = [
...
Tu misión es elevar el borrador a una propuesta lista para enviar, aplicando la rubrica PACT:
...
REGLA CRÍTICA: Devuelve la propuesta COMPLETA y mejorada. NUNCA recortes contenido."""),
        HumanMessage(content=f"""Revisa y mejora esta propuesta de marketing aplicando la rubrica PACT:
...
<style>
    ...
    .proposal-box h1, .proposal-box h2, .proposal-box h3 { color: #1a2f4b; }
...
with st.sidebar:
    ...
    api_key_input = st.text_input(
        ...
        type="password",
        placeholder="sk-ant-...",
        help="Ingresa tu API Key de Anthropic o defínela en el archivo .env",
    )
    st.markdown("---")
...
with col1:
    ...
    client_need = st.text_area(
        "Problema o necesidad de marketing",
        placeholder="Ej: Poca visibilidad online, redes sociales inactivas, sin estrategia digital...",
        height=100,
    )
...
with col2:
    ...
    service_details = st.text_area(
        "Descripción del servicio",
        placeholder="Ej: Gestión mensual de Instagram y Facebook: 3 publicaciones semanales, stories diarias, reportes mensuales...",
        height=100,
    )
    budget_range = st.text_input("Inversión mensual / total", placeholder="Ej: $2.500.000 COP / mes")
    timeline = st.text_input("Duración del proyecto", placeholder="Ej: Contrato inicial de 3 meses")
...
with col4:
    ...
    sender_contact = st.text_input("Correo / Teléfono", placeholder="Ej: carlos@creativehub.co · +57 300 000 0000")
...
if generate_btn:
    missing = []
    if not client_name:
        missing.append("Nombre del contacto")
    if not client_company:
        missing.append("Nombre de la empresa")
    if service_type == "Selecciona un servicio...":
        missing.append("Tipo de servicio")
        ...
        if not api_key:
            ...
            try:
                ...
                if "authentication" in err or "api_key" in err or "invalid" in err:
                    st.error("❌ API Key inválida. Verifica en [console.anthropic.com](https://console.anthropic.com).")
